                                        ...
                                        <div class="col-md-auto justify-content-end">
                                            <select class="form-select" name="formData[{{ $formIndex }}][{{ $index }}][{{ $indexItem }}][score]" id="score" required>
                                                ...
                                                @foreach ($ratings as $item) {{-- $isManager --}}
                                                    <option value="{{ $item['value'] }}" {{ isset($dataItem['score'][$indexItem]) && (isset($isManager) && $isManager || ($contributorTransaction ?? false) || ($sefInitiate ?? false)) && $dataItem['score'][$indexItem] == $item['value']  ? 'selected' : '' }}>{{ $item['value'] }}</option>
                                                @endforeach
                                            </select>
